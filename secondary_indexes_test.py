...
class TestSecondaryIndexes(Tester):
    ...
    def test_manual_rebuild_index(self):
        ...
        while not index_is_built(session, 'keyspace1', 'standard1', 'ix_c0'):
    ...
    @known_failure(failure_source='test',
                   jira_url='https://issues.apache.org/jira/browse/CASSANDRA-12098',
                   flaky=True)
    @since('3.0')
    ...
    def test_query_indexes_with_vnodes(self):
        ...
        start = time.time()
        while not index_is_built(session, 'ks', 'regular_table', 'composites_index') and time.time() + 10 < start:
            debug("waiting for index to build")
            time.sleep(1)
...
class TestSecondaryIndexesOnCollections(Tester):
    ...
    def test_map_indexes(self):
        ...
        while not index_is_built(session, 'map_index_search', 'users', 'user_uuids_values'):
            debug("waiting for index to build")
            time.sleep(1)

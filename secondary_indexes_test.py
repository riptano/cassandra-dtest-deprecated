...
class TestSecondaryIndexes(Tester):
    ...
    def test_manual_rebuild_index(self):
        ...
        while not index_is_built('keyspace1', 'standard1', 'ix_c0', 'session'):
    ...
    def test_query_indexes_with_vnodes(self):
        ...
        start = time.time()
        while not index_is_built('ks', 'regular_table', 'composites_index', 'session') and time.time() + 10 < start:
            debug("waiting for index to build")
            time.sleep(1)
...
class TestSecondaryIndexesOnCollections(Tester):
    ...
    def test_map_indexes(self):
        ...
        while not index_is_built('map_index_search', 'users', 'user_uuids_values', 'session'):
            debug("waiting for index to build")
            time.sleep(1)
